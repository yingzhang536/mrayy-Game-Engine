...
// 		shader->setVertexShaderConstant("viewPortSize",&sz.x,2);
		...
		shader->setTexture("tex0",tex0->GetTexture());
		shader->setTexture("tex1",tex1->GetTexture());
		shader->setTexture("tex2",tex2->GetTexture());
		shader->setTexture("tex3",tex3->GetTexture());
...
public:
	...
	virtual void setConstants(GCPtr<video::IGPUShaderProgram>shader)
	{
		...
		matWorldViewProj=worldView*matWorldViewProj;
...
class phongShaderConstants:public ShaderSetConstants
{
	...
	GCPtr<scene::CameraNode> m_cam;
public:
	phongShaderConstants(GCPtr<video::IVideoDevice> dev,video::SMaterial*mat,GCPtr<scene::LightNode> light,
		GCPtr<scene::CameraNode> cam)
	...
	virtual void setConstants(GCPtr<video::IGPUShaderProgram>shader)
	{
		...
		math::vector3d lp=m_light->getPosition();
		shader->setVertexShaderConstant("lightPos",&lp.x,3);
		shader->setVertexShaderConstant("camPos",&m_cam->getPosition().x,3);
...
class bumpShaderConstants:public ShaderSetConstants
{
	...
	GCPtr<scene::CameraNode> m_cam;
public:
	bumpShaderConstants(GCPtr<video::IVideoDevice> dev,video::SMaterial*mat,GCPtr<scene::LightNode> light,GCPtr<scene::CameraNode> cam)
		:ShaderSetConstants(dev,mat){
			m_light=light;
			m_cam=cam;
	}
...
public:
	...
	terrainShaderConstants(GCPtr<video::IVideoDevice> dev,video::SMaterial*mat,GCPtr<scene::CameraNode> cam)
		:ShaderSetConstants(dev,mat){
			fallOff=400;
			m_cam=cam;
	}
	...
	virtual void setConstants(GCPtr<video::IGPUShaderProgram>shader)
	{
		...
		math::vector3d pos=m_cam->getPosition();
