...
public:
	GrayscalePP(video::IVideoDevice*dev):IPostProcessing(dev)
	{
		...
		m_renderTarget=device->createRenderTarget(mT("GrayscaleRT"),device->getViewportRect().getSize(),video::EPixelFormat::EPixel_R8G8B8A8,false);
		m_callback=new PostprocessingShaderConstants(m_renderTarget->getColorTexture());
	...
	virtual GCPtr<video::IRenderTarget> render(GCPtr<video::IRenderTarget> input){
		...
		device->draw2DImage(math::rectf(-1,-1,1,1),input->getColorTexture(),video::SColor(255));
