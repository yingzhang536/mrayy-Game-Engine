...
const float CWiimote::ORIENT_PRECISION = WIIUSE_ORIENT_PRECISION;
/*
...
void CWiimote::SetLEDState(LEDS led,bool on)
{
	int st=GetLEDs();
	if(on)
	{
...
bool CWiimote::GetLEDState(LEDS led)
{
	return GetLEDs() & led;
}
...
CWii::CWii():mpWiimoteArray(0),mpWiimoteArraySize(0)
{
	SetMaxWiimotesCount(4);
}

CWii::CWii(int MaxNumWiimotes):mpWiimoteArray(0),mpWiimoteArraySize(0)
{
	SetMaxWiimotesCount(MaxNumWiimotes);
}

CWii::~CWii()
{
	for (int i=0;i<mpWiimotesVector.size();++i)
	{
		delete mpWiimotesVector[i];
	}
	wiiuse_cleanup((struct wiimote_t**) mpWiimoteArray, mpWiimoteArraySize);
}

void CWii::SetMaxWiimotesCount(int c)
{
	mpWiimoteArraySize = c;
	mpWiimoteArray = wiiuse_init(mpWiimoteArraySize);
	mpWiimotesVector.resize(mpWiimoteArraySize,0);
}
void CWii::RefreshWiimotes()
{
    ...
    for(index = 0; index < mpWiimoteArraySize; index++)
    {
		...
		{
			if(mpWiimotesVector[index])
				delete mpWiimotesVector[index];
			mpWiimotesVector[index]=new CWiimote(mpWiimoteArray[index]);
        }else
		{
			delete mpWiimotesVector[index];
			mpWiimotesVector[index]=0;
		}
    }
}
...
{
    int i= wiiuse_find((struct wiimote_t**) mpWiimoteArray, mpWiimoteArraySize, Timeout);
//	RefreshWiimotes();
	return i;
}

std::vector<CWiimote*>& CWii::ConnectWithAllMotes()
{
    int numConnected;
    int index;
...
CWiimote* CWii::Connect(int i)
{
	if(i>=mpWiimoteArraySize)return 0;

	if(mpWiimotesVector[i]){
		delete mpWiimotesVector[i];
		mpWiimotesVector[i]=0;
	}

	int cnt = wiiuse_connect((struct wiimote_t**) &mpWiimoteArray[i], 1);

	if(!cnt)return 0;

	mpWiimotesVector[i]=new CWiimote(mpWiimoteArray[i]);

	return mpWiimotesVector[i];
}

void CWii::DisconnectWithAllMotes()
{
	for (int i=0;i<mpWiimotesVector.size();++i)
	{
		delete mpWiimotesVector[i];
...
void CWii::Disconnect(int i)
{
	if(i>=mpWiimotesVector.size())return;
	delete mpWiimotesVector[i];
	mpWiimotesVector[i]=0;
}
int CWii::Poll()
