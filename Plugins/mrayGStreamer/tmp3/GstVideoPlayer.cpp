...
bool	GstVideoPlayer::Connect(const core::string& ip, int videoPort0, int videoPort1, int audioPort, int localAudioPort)
{
	...
	//gdpdepay !
	gstString = "appsrc name=src !application/x-rtp, payload=96 !   rtph264depay ! ffdec_h264! ffmpegcolorspace "
		"! video/x-raw-rgb !  appsink name = sink sync = false ";//"appsink name = sink sync = false";
	//" mysrc name=audioSrc !  audio/x-flac, channels=1, rate=8000! flacdec ! audio/x-raw-int,endianness=1234,signed=true,width=16,depth=16,rate=8000,channels=1 ! audioconvert ! autoaudiosink name=audioSink sync=false ";
	...
	gstString += "  udpsrc name=src1 port=" + core::StringConverter::toString(videoPort0) + " ! application/x-rtp, payload=96 !queue ! rtph264depay ! ffdec_h264 ! ffmpegcolorspace ! videoscale ! video/x-raw-rgb,width=" + core::StringConverter::toString(m_frameSize.x / 2) + ",height=" + core::StringConverter::toString(m_frameSize.y) + " ! videorate ! ffmpegcolorspace  ! mix.sink_1 ";
	gstString += "  udpsrc name=src2 port=" + core::StringConverter::toString(videoPort1) + " !application/x-rtp, payload=96 !queue ! rtph264depay !ffdec_h264 !ffmpegcolorspace ! videoscale ! video/x-raw-rgb,width=" + core::StringConverter::toString(m_frameSize.x / 2) + ", height=" + core::StringConverter::toString(m_frameSize.y) + " ! videorate ! videoflip method=5 ! videoflip method=4 !ffmpegcolorspace  !mix.sink_2 ";
