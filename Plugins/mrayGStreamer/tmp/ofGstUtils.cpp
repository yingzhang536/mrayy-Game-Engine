...
ofGstUtils::ofGstUtils() {
	...
	if(!gst_inited){
		gst_init (NULL, NULL);
		gst_inited=true;
		gLogManager.log("ofGstUtils: gstreamer inited",ELL_INFO);
	}
...
float ofGstUtils::getPosition(){
	if(gstPipeline){
		...
		if(!gst_element_query_position(GST_ELEMENT(gstPipeline),&format,&pos)){
			gLogManager.log("GStreamer: cannot query position",ELL_INFO);
			return -1;
		}
#else
...
ulong ofGstUtils::getDurationNanos(){
	...
	if(!gst_element_query_duration(getPipeline(),&format,&durationNanos))
		gLogManager.log("GStreamer: cannot query time duration",ELL_WARNING);
#else
	if(!gst_element_query_duration(getPipeline(),format,&durationNanos))
		gLogManager.log("GStreamer: cannot query time duration");
#endif
...
void ofGstUtils::setPosition(float pct){
	...
	if(speed>0){
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				...
				GST_SEEK_TYPE_SET,
				-1)) {
		gLogManager.log("GStreamer: unable to seek",ELL_WARNING);
		}
	}else{
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				flags,
...
void ofGstVideoUtils::update(){
	if (isLoaded()){
		...
		gLogManager.log("ofGstVideoUtils not loaded",ELL_WARNING);
	}
...
bool ofGstVideoUtils::setPipeline(const core::string& pipeline, video::EPixelFormat f, bool isStream, int w, int h){
	core::string caps;
	int bpp=video::PixelUtil::getPixelDescription(f).elemSizeB;
#if GST_VERSION_MAJOR==0
	if( bpp==8)
...
bool ofGstVideoUtils::allocate(int w, int h, int _bpp){
	...
	prevBuffer = 0;
	bHavePixelsChanged = 1;// pixels.isAllocated();
	return 1;
}
...
GstFlowReturn ofGstVideoUtils::preroll_cb(GstBuffer * _buffer){
	...
	if(pixels.imageDataSize && pixels.Size.x*pixels.Size.y*3/*pixels.getBytesPerPixel()*/!=(int)size){
...
GstFlowReturn ofGstVideoUtils::preroll_cb(GstSample * sample){
	...
	if(pixels.isAllocated()){
		...
		if(isStream && appsink){
			...
			gLogManager.log("received a preroll without allocation");
		}
		mutex.unlock();
	}
