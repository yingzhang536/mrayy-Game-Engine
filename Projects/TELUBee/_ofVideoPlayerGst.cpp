...
video::ImageInfo* ofVideoPlayerGst::getTextureReference(){
	//if(!tex.bAllocated() ){
		//ofLog(OF_LOG_WARNING, "ofVideoPlayerGst - getTextureReference - texture is not allocated");
//	}
	return &tex;
}
	...
	if (bLoaded == true){
		...
		//--------------------------------------------------------------
			...
			if(gstUtils.isFrameNew()){
				...
				tex.setData(gstUtils.getPixels(), math::vector3di(width, height,1), video::EPixel_R8G8B8);
			}
...
//---------------------------------------------------------------------------
bool ofVideoPlayerGst::loadMovie(core::string name){
...
//cout << "using GStreamer!" << endl;
	...
	//			tex.allocate(gstUtils.getWidth(),gstUtils.getHeight(),GL_RGB,false);
		//		tex.loadData(gstUtils.getPixels(), gstUtils.getWidth(), gstUtils.getHeight(), GL_RGB);
				tex.setData(gstUtils.getPixels(), math::vector3di(gstUtils.getWidth(), gstUtils.getHeight(),1), video::EPixel_R8G8B8);
			}
			bLoaded = true;
			allocated = true;
//			ofLog(OF_LOG_VERBOSE,"ofVideoPlayerGst: movie loaded");
	...
	//		ofLog(OF_LOG_ERROR,"ofVideoPlayerGst couldn't load movie");
			return false;
		}
...
bool	ofVideoPlayerGst::Connect(const core::string& ip,int remotePort,int localPort)
{
	if(!gstUtils.Connect(ip,remotePort,localPort))
		return false;
	if(bUseTexture){
		//			tex.allocate(gstUtils.getWidth(),gstUtils.getHeight(),GL_RGB,false);
		//		tex.loadData(gstUtils.getPixels(), gstUtils.getWidth(), gstUtils.getHeight(), GL_RGB);
