...
#define ofLog(X,MSG,Y,Z,G)printf("%s\n",MSG);
	...
	if ( hDevInfo ){
      while (TRUE){
         ...
         if (SetupDiGetDeviceRegistryPropertyA(hDevInfo,
			 ...
			 sprintf_s(portNamesFriendly[nPorts], MAX_PATH, "%s", dataBuf);
                ...
                if (end)
                    {
                      *end = 0;   // get rid of the )...
					  strcpy_s(portNamesShort[nPorts], MAX_PATH, begin);
                }
...
//----------------------------------------------------------------
int ofSerial::writeBytes(const unsigned char * buffer, int length){
