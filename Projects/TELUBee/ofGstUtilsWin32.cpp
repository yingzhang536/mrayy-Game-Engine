...
#define ofToString core::StringConverter::toString
void printVal(const core::string& v)
{
	printf("%s\n",v.c_str());
}
#define  ofLog(x,y) printVal(y);
...
static void get_video_devices (ofGstCamData & cam_data)
{
	...
	udev_list_entry_foreach(entry,list){
		const char * name = udev_list_entry_get_name(entry);
		struct udev_device * device = udev_device_new_from_syspath(my_udev, name);
		core::string subsystem = udev_device_get_subsystem(device);

		if(subsystem != "video4linux") continue;
		const char  *gstreamer_src, *product_name;
		struct v4l2_capability  v2cap;
		struct video_capability v1cap;
		core::string vendor_id;
		core::string product_id;
...
{
    core::string pipeline_desc = webcam_device.gstreamer_src + " name=source device=" +
...
bool ofGstUtils::loadMovie(core::string name){
	bpp 				= 3;
	bLoaded      		= false;
	bPaused 			= true;
	speed 				= 1.0f;
	bHavePixelsChanged 	= false;
	if( name.find( "://") == -1){
		name 			= core::string("file:///")+name;//+ofToDataPath(name,true);
		bIsStream		= false;
	}else{
		bIsStream		= true;
	}
...
bool	ofGstUtils::Connect(const core::string& ip,int remotePort,int localPort)
{
	...
	gstPipeline=pipeline = gst_pipeline_new( "pipeline0" );
	...
	gstSink = screen_sink            = gst_element_factory_make("appsink", NULL);//( "autovideosink", "screen_sink" );
	...
	gst_bin_add_many( GST_BIN( pipeline ), rtp_src, rtcp_src, rtcp_sink, depayloader, h264_decoder, csp_filter, NULL );
...
bool ofGstUtils::initGrabber(int w, int h, int framerate){
	...
	core::string format_str_pipeline = core::string("%s name=video_source device=%s ! ") +
...
bool ofGstUtils::setPipeline(core::string pipeline, int bpp, bool isStream){
	this->bpp = bpp;
	bHavePixelsChanged 	= false;
	bIsStream = isStream;
...
bool ofGstUtils::setPipelineWithSink(core::string pipeline){
	bHavePixelsChanged 	= false;
...
core::string getName(GstState state){
	switch(state){
