...
void GUIUserProfile::SetUser(kmd::KMDUser* u)
{
	m_user = u;
	if (m_user == 0)
	{
	...
	{
		m_profilePic = gTextureResourceManager.loadTexture2D( u->imageUrl);
		m_profilePic->setMipmapsFilter(false);
	}
}
kmd::KMDUser* GUIUserProfile::GetUser()
{
	return m_user;
}
...
void GUIUserProfile::Draw(const math::rectf*vp)
{
	...
	if (m_openAnimation == 1)
	{
		if (font && m_user)
		{
			...
			font->print(textRc, &m_fontAttrs, &clip, m_user->name, creator->GetRenderQueue());
