...
void ImageController::update(float dt){
	...
	if(m_currentImage==-1){
		...
		m_effector->reset();
		m_anchor->PosPercent=math::vector2d(math::Randomizer::rand01(),math::Randomizer::rand01());
	...
	if(m_effector->isDone()){
		...
		m_images[m_currentImage]->Visible=1;
		m_effector->reset();
		m_anchor->PosPercent=math::vector2d(math::Randomizer::rand01(),math::Randomizer::rand01())*0.8+0.1;
...
void ImageController::shuffle(){
	m_currentImage=-1;
	int cnt=m_images.size();
	for (int i=0;i<cnt-1;i++)
	{
		int id=math::Randomizer::rand(cnt-i-1);
