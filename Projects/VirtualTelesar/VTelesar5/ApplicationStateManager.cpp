...
void ApplicationStateManager::OnEvent(Event* e)
{
	BaseAppState* s=(BaseAppState*)m_stateMachine->getActiveState();
	if(!s)
		return ;
...
void ApplicationStateManager::Draw(bool left,const math::rectf& rc,video::IRenderTarget* rt)
{
	BaseAppState* s=(BaseAppState*)m_stateMachine->getActiveState();
	if(!s)
		return;
	s->GetState()->OnDraw(left,rc,rt);
}
