...
void RobotCommunicator::HandleData(network::NetAddress* addr,const core::string& name, const core::string& value)
{
	...
	{
		int videoPort0 = atoi(vals[1].c_str());
		int videoPort1 = atoi(vals[2].c_str());
		int audioPort = atoi(vals[3].c_str());
		network::NetAddress addr = network::NetAddress(vals[0], videoPort0);
		//if (addr.address != m_userStatus.address.address || addr.port!=m_userStatus.address.port)
		{
			m_userStatus.address = addr;
			if (m_listener)
			{
				m_listener->OnUserConnected(this, m_userStatus.address, videoPort0, videoPort1, audioPort);
			}
		}
	}
...
void RobotCommunicator::ProcessPacket(network::NetAddress* addr,const char* buffer)
{
	...
	if (m_robotStatus.connected && !m_robotController->GetRobotController()->IsConnected())
