...
public:
	...
	network::IUDPClient* m_videoClient[2];
	network::IUDPClient* m_audioClient;
...
#define SENDING_BLOCK 4096
	...
	class CStreamingData
	{
	public:
		GstVideoProviderImpl* o;
		int port;
		network::IUDPClient* client;
	};

	CStreamingData m_videoSD[2];
	CStreamingData m_audioSD;
public:
	...
	* We remove the idle handler from the mainloop */
	...
	//	buffer = gst_app_sink_pull_buffer(GST_APP_SINK(sink));
		...
		CStreamingData* sd = ((CStreamingData*)data);
		if (buffer) {
			/* The only thing we do in this example is print a * to indicate a received buffer */
			//	Sleep(10);
			sd->o->SendData(buffer,sd);
			//gst_buffer_unref(buffer);
		}
		return GST_FLOW_OK;
	}
...
public:
	...
	void SendData(GstBuffer* buffer,CStreamingData*sd)
	{
#if GST_VERSION_MAJOR==0
		...
		addr.port = sd->port;
		sd->client->SendTo(&addr, (char*)data, size);
		m_sentBytes += size;
//		printf("data sent: %d\n", size);
	...
	//	printf("%d-",size);
		...
		while (remaining > 0)
		{
			...
			{
				sd->client->SendTo(&m_targetAddress, (char*)m_sendingBuffer, SENDING_BLOCK);
				m_dataOffset = 0;
			}
		}
	}
	...
	void SetCameraSource(int left, int right)
	{
		m_leftCamera = left;
		m_rightCamera = right;
	}
	...
	void StreamDataTo(const network::NetAddress& target, int videoPort1, int videoPort2, int audioPort)
	{
		m_targetAddress = target;
		m_videoPorts[0] = videoPort1;
		m_videoPorts[1] = videoPort2;
	...
	bool Start(EStreamingQuality quality)
	{
		...
		math::vector2d m_frameSize(1280, 720);
		...
		gstString = "videomixer name=mix "
			...
			"x264enc  name=enc"
			" pass=" + pass +
			"  qp-max=" + core::StringConverter::toString(quanizer) +
			//"  quantizer=" + core::StringConverter::toString(quanizer) +
			//	"   speed-preset=ultrafast sliced-threads=false bitrate=" + core::StringConverter::toString(bitrate) +
			" tune=zerolatency"
			//" rc-lookahead=1"
			...
			//	" !gdppay" 
			" ! mysink name=sink sync=false ";
		gstString += " videotestsrc pattern=\"black\" ! video/x-raw-rgb,width=1280,height=720  !ffmpegcolorspace ! mix.sink_0 ";
		gstString += "  ksvideosrc device-index=0 ! video/x-raw-rgb,width=1280,height=720 !ffmpegcolorspace ! videoscale ! video/x-raw-rgb,width=" + core::StringConverter::toString(m_frameSize.x / 2) + ",height=" + core::StringConverter::toString(m_frameSize.y) + "   ! mix.sink_1 ";
		gstString += "  ksvideosrc device-index=1 ! video/x-raw-rgb,width=1280,height=720 !ffmpegcolorspace ! videoscale ! video/x-raw-rgb,width=" + core::StringConverter::toString(m_frameSize.x / 2) + ", height=" + core::StringConverter::toString(m_frameSize.y) + "  ! mix.sink_2 ";


		gstString = " ksvideosrc device-index=0 ! video/x-raw-rgb, width=1280,height=720,framerate=25/1 ! queue  ! ffmpegcolorspace ! videorate !  videoscale ! video/x-raw-yuv,width=640,height=720 !"
			"x264enc  name=enc"
			" pass=" + pass +
			"  qp-max=" + core::StringConverter::toString(quanizer) +
			//"  quantizer=" + core::StringConverter::toString(quanizer) +
		...
		gstString = " videomixer name=mix sink_0::xpos=0   sink_0::ypos=0  sink_0::alpha=1  sink_0::zorder=0  sink_1::xpos=0   sink_1::ypos=0  sink_1::zorder=1     sink_2::xpos=640   sink_2::ypos=0  sink_2::zorder=1 "
			"   ! ffmpegcolorspace ! "
			...
			//	" !gdppay" 
			" ! mysink name=sink sync=false "
			"     videotestsrc pattern=\"black\" ! video/x-raw-yuv,width=1280,height=720  !ffmpegcolorspace ! mix.sink_0 "
			" ksvideosrc device-index=0 ! video/x-raw-yuv,width=1280,height=720,framerate=25/1 !ffmpegcolorspace! videorate ! videoscale ! video/x-raw-yuv,width=640,height=720 ! mix.sink_1  "
			"   ksvideosrc device-index=1 ! video/x-raw-yuv,width=1280,height=720,framerate=25/1 ! ffmpegcolorspace ! videorate !  videoscale ! video/x-raw-yuv,width=640,height=720 !  mix.sink_2";
		...
		gstString = " videomixer name=mix  "
			...
			//	" !gdppay" 
			" ! mysink name=sink sync=false "
			" ksvideosrc device-index=0 ! video/x-raw-yuv,width=1280,height=720,framerate=25/1 !ffmpegcolorspace! videorate ! videoscale ! video/x-raw-yuv,width=640,height=720 ! videobox border-alpha=0 top=0 left=0 ! mix.  "
			" ksvideosrc device-index=1 ! video/x-raw-yuv,width=1280,height=720,framerate=25/1 ! ffmpegcolorspace ! videorate !  videoscale ! video/x-raw-yuv,width=640,height=720 ! videobox border-alpha=0 top=0 left=640!  mix.";


		int targetCam[2]={m_leftCamera,m_rightCamera};
		gstString = "";
		for(int i=0;i<2;++i)
		{

			gstString += " ksvideosrc device-index=" + core::StringConverter::toString(targetCam[i]) + 
				" ! video/x-raw-rgb, width=1280,height=720,framerate=25/1 ! queue  !  videorate !  videoscale ! video/x-raw-rgb,width=640,height=720 ! ffmpegcolorspace !"
				"x264enc  name=enc" + core::StringConverter::toString(i)+
				" pass=" + pass +
				"  qp-max=" + core::StringConverter::toString(quanizer) +
				//"  quantizer=" + core::StringConverter::toString(quanizer) +
				//	"   speed-preset=ultrafast sliced-threads=false bitrate=" + core::StringConverter::toString(bitrate) +
				...
				"  ! rtph264pay"// mtu=" + core::StringConverter::toString(mu) + 
				//	" !gdppay" 
				" ! mysink name=sink"+core::StringConverter::toString(i)+" sync=false ";
		}
#else
...
// 			" tune=zerolatency"
		...
		GstMySink* sink0 = GST_MYSINK(gst_bin_get_by_name(GST_BIN(gstPipeline), "sink0"));
		GstMySink* sink1 = GST_MYSINK(gst_bin_get_by_name(GST_BIN(gstPipeline), "sink1"));
...
// 		g_signal_connect(gstSrc, "enough-data", G_CALLBACK(stop_feed), this);
		...
		if (sink0)
		{
			sink0->new_buffer = new_buffer;
			sink0->data = &m_videoSD[0];
		}
		if (sink1)
		{
			sink1->new_buffer = new_buffer;
			sink1->data = &m_videoSD[1];
		}
		if (audioSink)
		{
			audioSink->new_buffer = new_buffer;
			audioSink->data = &m_audioSD;
		}
#if 0
		...
		GstMySink* sinks[3] = { sink0, sink1, audioSink };
		for (int i = 0; i < 3; ++i)
		{
			g_object_set(sinks[i], "emit-signals", false, "sync", false, NULL);
			g_object_set(G_OBJECT(sinks[i]), "sync", FALSE, "async", FALSE, (void*)NULL);
			g_object_set(sinks[i], "emit-signals", false, NULL);


			gst_base_sink_set_sync(GST_BASE_SINK(sinks[i]), false);
			gst_app_sink_set_max_buffers(GST_APP_SINK(sinks[i]), -1);
			gst_app_sink_set_drop(GST_APP_SINK(sinks[i]), true);
			gst_base_sink_set_max_lateness(GST_BASE_SINK(sinks[i]), -1);
		}
...
//		gst_caps_unref(caps);
		...
		try
		{
			for (int i = 0; i < 2; ++i)
			{
				m_videoClient[i] = network::INetwork::getInstance().createUDPClient();
				m_videoClient[i]->Open();
				m_videoSD[i].client = m_videoClient[i];
				m_videoSD[i].o = this;
				m_videoSD[i].port = m_videoPorts[i];
			}

			m_audioClient = network::INetwork::getInstance().createUDPClient();
			m_audioClient->Open();
			m_audioSD.client = m_audioClient;
			m_audioSD.o = this;
			m_audioSD.port = m_audioPort;
		}
		...
		setPipelineWithSink(gstPipeline,  true);
		if (!startPipeline())
		{
	...
	void Stop()
	{
		if (!bLoaded)
			return;
		close();
		delete m_videoClient[0];
		delete m_videoClient[1];
		delete m_audioClient;
		m_videoClient[0] = 0;
		m_videoClient[1] = 0;
		m_audioClient = 0;
	...
	bool setPipelineWithSink(GstElement * pipeline, bool isStream_){
		startGstMainLoop();
...
void GstVideoProvider::SetDebug(bool debug)
{
	m_impl->SetDebug(debug);
}
void GstVideoProvider::SetCameraSource(int left, int right)
{
	m_impl->SetCameraSource(left, right);
}
void GstVideoProvider::StreamDataTo(const network::NetAddress& target, int videoPort0, int videoPort1, int audioPort)
{
	m_impl->StreamDataTo(target,videoPort0,videoPort1,audioPort);
}
