...
public:
	...
	virtual void update(float dt){
		if(Visible()){
			float dx=m_speed*(gKeyboardController.getKeyState(KEY_D)-
				gKeyboardController.getKeyState(KEY_A))*dt;
	...
	// 			dz*=(1+2*gKeyboardController.isLShiftPress());
			...
			if(gMouseController.isPressed(controllers::EMB_Right)){
				this->rotate(30*gMouseController.getDX()*dt,math::vector3d(0,1,0),scene::TS_World);
				this->rotate(30*gMouseController.getDY()*dt,math::vector3d(1,0,0),scene::TS_Local);
			}
