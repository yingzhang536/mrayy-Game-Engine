...
IMPLEMENT_ELEMENT_FACTORY(GUIListBox);


GUIListBox::GUIListBox(IGUIManager*mngr):IGUIListBox(mngr)
{
	m_component=new GUIListBoxComponent(this);
...
IListItem* GUIListBox::GetItem(int index)const
{
	if(index<0 || index>=m_component->items.size())return 0;
	return m_component->items[index];
}
int GUIListBox::FindItem(const core::UTFString& txt)const
{
...
int GUIListBox::AddItem(IListItem* item)
{
	m_component->items.push_back(item);
	return m_component->items.size()-1;
}
int GUIListBox::InsertItem(int index,IListItem* item)
{
...
const ItemList& GUIListBox::GetItems()const
{
	return m_component->items;
}
void GUIListBox::SortItems()
{
	for(int i=0;i<m_component->items.size();++i)
	{
...
void GUIListBox::SetSelectionColor(const video::SColor& clr)
{
	m_component->SetSelectionColor(clr);
}

const video::SColor& GUIListBox::GetSelectionColor()const
{
...
IGUIElement* GUIListBox::Duplicate()
{
	return new GUIListBox(GetCreator());
}
FontAttributes* GUIListBox::GetFontAttributes()
{
...
bool GUIListBox::_OnKeyboardEvent(KeyboardEvent* e)
{
	if(GetCreator()->HasFocus(this))
	{
		GUIListBoxComponent::EResultEvent r=m_component->LBOnKeyboardEvent(e,m_component->rc);
		if(r==GUIListBoxComponent::ESelectionChange)
		{
			int si=m_component->GetSelectedItem();
...
bool GUIListBox::_OnMouseEvent(MouseEvent* e)
{
	...
	GUIListBoxComponent::EResultEvent r=m_component->LBOnMouseEvent(e,m_component->rc);
	if(r==GUIListBoxComponent::ENone)
		return false;
...
void GUIListBox::_notifyLostFocus()
{
...
void GUIListBox::onEvent(const EventPtr& e)
{
	if(!Visible)return;
	if(!Enable)return;
...
int GUIListBox::getItemFromPos(float x,float y)
{
	IGUITheme* skin=GUIThemeManager::getInstance().getActiveTheme(); 	
	IFont*font=gFontResourceManager.getFontByName(m_FontAttributs.fontName);
	if(!font)return -1;
...
GUIListBox::listItem* GUIListBox::getItem(int index)
{
	if(index>=itemList.size())return 0;

	return &itemList[index];
}
int GUIListBox::getItemIndex(const core::UTFString& item)
{
...
int GUIListBox::addItem(const core::UTFString& item,const video::ImageSetTexture& icon,void*userData)
{
	itemList.push_back(listItem(item,icon,userData));
	return itemList.size()-1;
}

void GUIListBox::setItem(int id,const core::UTFString& item,const video::ImageSetTexture& icon,void*userData)
{
...
void GUIListBox::deleteItem(const core::UTFString& elem)
{
...
GCPtr<GUIElement> GUIListBox::getDuplicate()
{
	GUIListBox*dup=guiMngr->addList(getPos(),getSize(),m_event,getParent(),Color(),getId());
